       ...
       for ($i = 0; $i -lt $certs.count; $i++)
        {
            ...
            if ($Template -eq $null)
            { 
                $Template = "TemplateNameNotAvailable" 
            }
            else
            {
                # String will usually be like "Template=MachineAuth(1.3.6.1.4...."
                $Template = $Template.Format($False) -Replace 'Template=',''
                # But sometimes there is no open paren:
                $ParenPosition = $Template.IndexOf('(')
                if ($ParenPosition -eq -1){ $ParenPosition = $Template.Length } 
                $Template = $Template.SubString(0,$ParenPosition) 
            }
