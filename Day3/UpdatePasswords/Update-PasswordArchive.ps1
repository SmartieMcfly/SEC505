...
#    script is also not compatible with FIPS Mode being enabled in Windows.  The
#    script is currently not compatible with PowerShell Core (pwsh.exe).
#
...
# Exit if PowerShell Core (pwsh.exe) is being used.
if ($PSVersionTable.PSEdition -ne "Desktop")
{ Write-Error -Message "This script is incompatible with PowerShell Core (pwsh).  Use Windows PowerShell instead." -ErrorAction Stop }
